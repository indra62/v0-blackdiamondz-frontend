...
// Submits newsletter email to HubSpot via internal API
export async function submitNewsletterSubscribe(formData) {
  try {
    const response = await fetch("/api/hubspot-newsletter", {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || "Newsletter subscribe failed");
    }
    return await response.json();
  } catch (error) {
    console.error("Error submitting newsletter subscribe", error);
    throw error;
  }
}
...
export async function submitPropertySellSubmission(formData) {
  try {
    const [propertyApiResponse, hubspotApiResponse] = await Promise.all([
      api.post("/items/property_sale_submission", {
        first_name: formData.first_name,
        last_name: formData.last_name,
        email: formData.email,
        phone: formData.phone,
      ...
      api.post("http://localhost:3000/api/hubspot-contact", {
        email: formData.email,
    ...
    return propertyApiResponse.data.data;
  } catch (error) {
    console.error("Error submitting property sell submission or HubSpot contact:", error);
    throw error;
  }
}
