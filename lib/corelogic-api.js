...
export async function getCoreLogicReport(token, propertyId) {
  try {
    const url = "https://api-sbox.corelogic.asia/property-profile-report/au/property/" + propertyId + "/ppr";
    // Hardcoded request body as per user instruction
    const requestBody = {
      "customerName": "nandha",
      "reportRequester": {
        "email": "nandha@62teknologi.com",
        "name": "nandha",
        "phone": "1234567890"
      }
    };
    ...
    console.log("CoreLogic report response:", response.data);
    console.log("CoreLogic report response:", JSON.stringify(response.data, null, 2));
    
    return response.data;
  } catch (error) {
    console.error("Failed to get CoreLogic report:", error.response?.data || error.message);
    throw error;
  }
}
