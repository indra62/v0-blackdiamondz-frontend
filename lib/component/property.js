...
export function Property({ property, taviraj, archivo, dark = true, savedPropertyId, refreshSavedProperties }) {
  ...
  return (
    <>
      {showMap && (
        ...
        >
          <div style={{ position: "relative", width: "90vw", height: "90vh",  borderRadius: "8px", overflow: "hidden" }}>
            <button
      ...
      <div>
        ...
        <div className="flex justify-between items-center mb-2">
          <div
            className={`${archivo.className} ${
              darkMode ? "text-[#BD9574]" : "text-[#9A795F]"
            } font-light text-[16px] leading-[150%]`}
          >
            {translation?.name || ""}
          </div>
          <HeartButton propertyId={property.id} uniqueId={property.unique_id} mode={mode} savedPropertyId={savedPropertyId} refreshSavedProperties={refreshSavedProperties} />
        </div>
        ...
        >
          <h3
            className={`${
              taviraj.className
            } text-[24px] font-light leading-[40px] mb-2 h-20 overflow-hidden ${
        ...
        <div
          className={`${
            archivo.className
          } text-[16px] font-bold leading-[150%] mb-6 ${
            !darkMode ? "text-[#211F17]" : "text-[#E2DBCC]"
          }`}
        >
          {property?.is_auction && (
            <span
              className={`font-light ${
                mode === "light" ? "text-[#9A795F]" : "text-[#BD9574]"
              }`}
            >
              Auction:{" "}
            </span>
          ...
          <button
            className={`py-4 flex items-center justify-center gap-2 ${
              mode === "light"
                ? "text-[#211F17] hover:text-[#BD9574]"
                : "text-[#BD9574] hover:text-[#eea161]"
            } border border-[#656565] hover:border-[#BD9574] hover:text-[#BD9574] transition-colors`}
          >
